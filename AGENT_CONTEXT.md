...
- **Numbered fixes**: every commit message starts with `B<n>:`. The current series is at **B27** (the AMD EPYC 7763 re-baseline + 16-way file concurrency + MemoryLimit defaults commit). Pick the next free number (B28 at time of writing) when committing a new fix or feature. Use `git log --oneline | Select-Object -First 1` to confirm the current head before picking a number.
...
- **2026-04-27** — B27 commit landed. The 2026-04-25 overnight run completed cleanly after the user adopted recommendation 1 (`InMemoryBlobService.retainPayloads=false`) on top of the trimmed 7-class filter list; final wall-clock ~42 hours, all 7 target benchmarks produced reports, no OOM, `DONE` marker recorded. Three findings from the data drove this commit: (1) `TwoTierFileSplitBigScaleBenchmark` showed 16-way file concurrency wins -22 to -27% on the two production-shaped big workloads with no regression elsewhere; B27a is shipped as a 1-line change to the `MaxParallelFileBackups` constant. (2) `MemoryBudgetBenchmark` showed all values within 4.5% of unlimited on `media-library-500`, so constraining is free; B27d is shipped as a config-default flip (`MemoryLimitEnabled=true`, `MemoryLimitMB=8192`). The 8 GB choice (lower than the AGENT_CONTEXT-suggested 16 GB) is justified in the new xmldoc on the property: the 16-way file-concurrency ceiling under B27a is `16 x 6 x 64 MB = 6 GB`, so 8 GB is the smallest stepped value that does not throttle. (3) `LargestFirstSchedulingBenchmark` reproduced the small-workload regressions on this hardware (+19% on `mixed-realistic-1000`, +24% on `realistic-large-200`); the `BigScale` companion was flat at big scale; W2 conclusion is "do nothing, blanket LPT remains DO-NOT-DO." Architectural fact #11 added documenting the new `InMemoryBlobService.retainPayloads` constructor parameter and the `BackupBenchmarkBase.RetainBlobPayloads` virtual hook -- the discard-mode seam that removed the RAM ceiling that had caused the first B27 hardware run (2026-04-24) to OOM. Fact #10 got a one-line correction note: the `MemoryLimitMBOverride` seam works as designed; the OOM was caused by the destination side of the upload, not the budget. Both `docs/SETUP.md` (technical-specs table now lists `MaxParallelFileBackups=16`, `MaxParallelChunkUploads=6`, `MemoryLimitEnabled=true`, `MemoryLimitMB=8192`) and `docs/USER_GUIDE.md` (Memory limit section now describes the on/8 GB default) updated in the same commit per the verified-current-docs maintenance protocol. Result tables filled in all 7 benchmark xmldocs -- pre-B27 small benchmarks (`BackupThroughputBenchmark`, `TwoTierFileSplitBenchmark`, `LargestFirstSchedulingBenchmark`) got a new B27 same-hardware block APPENDED below the historical i7-9700K block (per the same-hardware discipline rule, the original blocks are preserved as documented different-hardware data); the four big-scale benchmarks got their previously empty placeholder tables filled. The `LargestFirstSchedulingBigScaleBenchmark.media-library-500` row is flagged with an asterisk -- one of its two iterations had a 9-hour pathological tail almost certainly caused by host-OS pressure during the 42-hour overnight, not by the orchestrator; the LPT iteration on the same workload was a stable ~5 m, so the cell is recorded with a caveat rather than left empty. Active-workstreams section now reads "None" with four deferred follow-ups itemized; W1 and W2 moved to Completed workstreams. No emoji, no non-ASCII; commit prefix `B27:`. Author: Copilot agent session.
